...
class RapidOCR:
    def __init__(
        ...
        det_use_cuda: Optional[bool] = None,
        cls_use_cuda: Optional[bool] = None,
        rec_use_cuda: Optional[bool] = None,
        **kwargs
    ):
        # Handle legacy CUDA parameters for backward compatibility
        if params is None:
            params = {}
        
        # Map legacy parameters to new config structure
        if det_use_cuda is not None:
            params["EngineConfig.onnxruntime.use_cuda"] = det_use_cuda
        if cls_use_cuda is not None:
            # Note: cls uses OpenVINO by default, but for backward compatibility
            # we'll keep the parameter even if it doesn't apply
            pass
        if rec_use_cuda is not None:
            if "EngineConfig.onnxruntime.use_cuda" not in params:
                params["EngineConfig.onnxruntime.use_cuda"] = rec_use_cuda
